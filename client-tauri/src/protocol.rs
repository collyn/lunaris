...
#[cfg(target_os = "linux")]
fn register_linux(exe_path: &str) -> Result<(), Box<dyn std::error::Error>> {
...
"#,
    ...
    println!("Registered desktop entry at {:?}", dest_file);
    ...
    match status {
        Ok(s) if s.success() => {
            println!("Registered lunaris URI scheme handler successfully");
        }
        other => {
            eprintln!("Failed to run xdg-mime register: {:?}", other);
        }
    }
...
#[cfg(target_os = "windows")]
fn register_windows(exe_path: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Registered Windows registry entry for lunaris://");
    Ok(())
}
