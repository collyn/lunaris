...
fn main() {
    ...
    if parsed_args.is_none() {
        eprintln!(
            "Usage: client-qml --host-id <HOST_ID> --server <SERVER_URL> --token <JWT_TOKEN> [options]\n\n\
             Options:\n  \
               --res <w>x<h>             Resolution (e.g. 1920x1080, default 1280x720)\n  \
               --fps <fps>               FPS (default 60)\n  \
               --bitrate <kbps>          Bitrate in kbps (default 8000)\n  \
               --codec <codec>           Video codec h264/h265/av1 (default h264)\n  \
               --mouse-queue-limit <lim> Mouse queue buffer limit in bytes (default 256)\n\n\
             Or deep-link connect:\n\
               lunaris://connect?host_id=<HOST_ID>&server=<SERVER_URL>&token=<JWT_TOKEN>..."
        );
        std::process::exit(1);
    }
