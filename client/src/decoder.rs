...
// Make the raw pointers Send since they are only used within a single Tokio task thread
unsafe impl Send for HardwareDecoder {}

impl HardwareDecoder {
    pub fn new(codec_type: CodecType) -> Result<Self, anyhow::Error> {
        // Initialize FFmpeg library logging
        unsafe {
        ...
        unsafe {
            ...
            // Use 1 thread for ultra-low latency real-time decoding
            (*codec_ctx).thread_count = 1; 
        }

        // Detect target platform and initialize corresponding GPU hardware acceleration device context
        ...
        if hw_device_type != ffi::AVHWDeviceType::AV_HWDEVICE_TYPE_NONE {
            ...
            // Fallback for Windows if D3D11VA fails, try DXVA2
    ...
    fn process_frame(&mut self, gpu_frame: *mut ffi::AVFrame) -> Result<YUVFrame, anyhow::Error> {
        // Determine if frame needs to be copied from GPU memory (VRAM) to CPU system memory (RAM)
        ...
        unsafe {
            if !(*gpu_frame).hw_frames_ctx.is_null() {
                ...
                if transfer_err >= 0 {
                    ...
                    // If transfer fails, attempt to read directly from gpu_frame (though likely empty or formatted as NV12/VAAPI surface)
                }
            }
        }

        // Ensure the texture is converted and scaled to standard YUV420P format using libswscale
        ...
        // Copy raw pixel arrays to compacted YUV vectors
        ...
        // Cleanup intermediate frame allocations
        unsafe {
            ffi::av_frame_free(&mut (dst_frame as *mut _));
            if is_hw_frame {
                ffi::av_frame_free(&mut (cpu_frame as *mut _));
