...
fn latest_child_dir(base: &Path) -> Option<PathBuf> {
...
fn add_latest_msvc_include(paths: &mut Vec<PathBuf>, vs_path: &Path) {
    let msvc_base = vs_path.join("VC").join("Tools").join("MSVC");
    if let Some(msvc_version) = latest_child_dir(&msvc_base) {
        push_existing_unique_path(paths, msvc_version.join("include"));
    }
}

fn add_latest_windows_sdk_includes(paths: &mut Vec<PathBuf>, program_files_x86: &Path) {
    let sdk_base = program_files_x86
        .join("Windows Kits")
        .join("10")
        .join("Include");
    if let Some(sdk_version) = latest_child_dir(&sdk_base) {
        for subdir in ["ucrt", "shared", "um", "winrt"] {
            push_existing_unique_path(paths, sdk_version.join(subdir));
        }
    }
}

fn add_windows_msvc_bindgen_includes(clang_includes: &mut Vec<String>) {
    ...
    if let Some(include) = env::var_os("INCLUDE") {
        for path in env::split_paths(&include) {
            push_existing_unique_path(&mut include_paths, path);
        }
    }
        ...
        if vswhere.exists() {
                ...
                .output()
            {
                if output.status.success() {
                    ...
                    if !vs_path.is_empty() {
                        add_latest_msvc_include(&mut include_paths, Path::new(&vs_path));
                    }
                }
            }
        }

        add_latest_windows_sdk_includes(&mut include_paths, &program_files_x86);
    }

    if let Some(program_files) = env::var_os("ProgramFiles").map(PathBuf::from) {
        let vs_2022_base = program_files.join("Microsoft Visual Studio").join("2022");
        for edition in ["Community", "Professional", "Enterprise", "BuildTools"] {
            add_latest_msvc_include(&mut include_paths, &vs_2022_base.join(edition));
        }
    }

    let mut has_ucrt = false;
    for path in include_paths {
        if path.to_string_lossy().contains("Windows Kits") && path.ends_with("ucrt") {
            has_ucrt = true;
        }

        let arg = format!("-I{}", path.to_string_lossy());
        if !clang_includes.iter().any(|existing| existing == &arg) {
            println!("cargo:rerun-if-changed={}", path.display());
            clang_includes.push(arg);
        }
    }

    if !has_ucrt {
        println!("cargo:warning=Windows SDK UCRT include path was not found; bindgen may fail to resolve errno.h");
    }
}
...
fn main() {
    ...
    let mut clang_includes = include_paths
        ...
        .collect::<Vec<_>>();
    add_windows_msvc_bindgen_includes(&mut clang_includes);
